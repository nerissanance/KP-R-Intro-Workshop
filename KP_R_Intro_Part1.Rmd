...
```{r, eval=FALSE}
install.packages(c("knitr", "psych", "rmarkdown", "swirl", "mlbench", "tidyverse"))
```
...
We'll use this more in parts 2, but take a moment to look at the data you imported. You can use the `hist()` command to do a histogram and the `summary()` command to look at summary statistics. For example:
