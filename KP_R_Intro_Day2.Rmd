...
---
 #Citations:
A lot of this material came directly from D-lab training materials authored by D-Lab staff. The original content can be accessed here: [https://github.com/dlab-berkeley/R-Fundamentals]
...
4. Missing data (NA)  
5. Merging data frames in dplyr  
...
# **Challenge 2**
1. Practice extracting a few variables from the `df.pid` dataframe using the dollar sign `$`
...
### reshaping basics with dplyr 
For a full version of this tutorial, see the dplyr CRAN page: [https://cran.r-project.org/web/packages/dplyr/vignettes/dplyr.html]

```{r dplyr tutorial}
library(dplyr)
library(tidyr)
#load data, in this case, it comes as a package
...
####Spread by month and year
spread <- spread(flights, month, year)
...
####Unite
flights$date <- unite(flights, date, month, day, year, sep="/")
names(flights)
