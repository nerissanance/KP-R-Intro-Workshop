...
A project's folder is automatically set as the working directory, which allows us to not worry about explicitly setting the working directory. This can be more convenient when working on the same code across different computers. 
...
As an alternative to an R script we can use a markdown file. R Markdown files combine R scripts with simple text formatting, which makes reports  (PDFs or HTMLs) easy to generate with embedded R output. Please take a second now to run `install.packages("markdown")` to install it. 
...
welcome_object <- "Welcome to Training Day 1"
ls() 
```
...
Character data have some useful functions such as  `substr` and `strsplit`. We'll use these to explore function arguments. Use your help commands to learn more about the commands!  
...
##Importing csv and SAS data files
...
See where your data frame is saved by typing(note: since we're using R projects, you should already know where your wd is, because it's set up automatically): 
```{r}
getwd()
```
...
The Rio package simplifies importing/exporting by putting all data-specific import tools under one umbrella (a.k.a. the "swiss army knife" of data importing and exporting)
...
2. Read in the csv file of the WHO data using the rio package and compare the results to the above SAS import.
```{r}
## YOUR CODE HERE
...
- Contributions by Dlab affiliates Evan Muzzall, Rochelle Terman, Dillon Niederhut, Sam Abdel-Ghaffar