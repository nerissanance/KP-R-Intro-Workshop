---
title: 'KP R Intro, Part 2: Data manipulation, summarization and graphing'
...
* 2. Missing data (NA)  
* 3. Intro to dplyr: subsetting, filtering, merging
...
### Subsetting with logical tests in base r
...
Dplyr is a powerful package from 'tidyverse' (a group of packages created/maintained by, among others, the statistician Hadley Wickham). It has a lot of really powerful commands for data manipulation and summary techniques. I highly recommend downloading the cheatsheet, accessed (here)[https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf]. 
...
You can also subset using the 'filter' command in dplyr:
```{r}

all41ordiab2 <- df.pid %>% filter(df.pid$age == 41 | df.pid$diabetes == "pos")
head(all41ordiab2)
...
summary(df.pid$glucose)
```



# Data summarization using dplyr
...
df.pid %>% select (glucose) %>% summarize(mean_g=mean(glucose))

#need to tell dplyr to remove NAs!

df.pid %>% select (glucose) %>% summarize(mean_g=mean(glucose, na.rm=TRUE))

# let's look at the distribution of all the numeric variables at once using the `summarise_if` function of dplyr:
head(df.pid)
summarise_if(df.pid, is.numeric, funs(mean,median), na.rm = TRUE)

# you can also group by a certain variable, like diabetes status, using the 'group_by()' command

 df.pid %>% group_by(diabetes) %>% summarise(mean_gluc=mean(glucose,na.rm = TRUE))
...
df.pid %>% mutate(gbp_ratio= pressure/glucose)

#this prints the data, but we need to assign it in order to save it:
df.pid  <- df.pid %>% mutate(gbp_ratio= pressure/glucose)


summary(df.pid$gbp_ratio)
...
2) Keep only the statistics for Albania 
3) Summarize the WHO suicide numbers by gender 
...
Extra credit #2: How might you collapse the data to make it more useful? (hint: check out the cheatsheet for the dplyr packge for help)
```{r}
#YOUR CODE HERE
