...
Packages can also be installed through the command line using `install.packages()`. The package name must be wrapped in quotation marks so that R knows it is searching for that particular package named `"swirl"`, rather than previously defined data named swirl:
...
Convention: we **highly** recommend that you do not use caps when naming objects. It makes it more cubersome to call objects later, and can lead to confusion and misnaming.
...
Character data have some useful functions such as  `substr` ands `strsplit`. We'll use these to explore function arguments. Use your help commands to learn more about the commands!  
...
Extra credit: Use `substr` to extract the first word of your sentence (hint: look at the help file for usage)
...
###Importing csv data files using `read.csv`
...
We'll use this more in part 2, but take a moment to look at the data you imported. You can use the `hist()` command to do a histogram and the `summary()` command to look at summary statistics. For example:
